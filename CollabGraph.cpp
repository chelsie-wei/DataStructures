...
void CollabGraph::populate_graph(const vector<Artist>& artists) 
{
    ...
    for (int j = 0; j < vecSize; j++) {
        for (int k = 0; k < vecSize; k++) {
            string collab = artists[j].get_collaboration(artists[k]);
            
            if ((j != k) and (collab != "")) 
                insert_edge(artists[j], artists[k], collab);  
        }
    }
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const 
{
    ...
    Vertex *ver = graph.at(artist.get_name());
    int size = ver->neighbors.size();  // size of the vector of edges in ver
    int i = 0;
    
    while (i < size) { 
        neighbors.push_back(ver->neighbors[i].neighbor->artist);
        i++;
    }
                                       ...
                                       const Artist& dest) const 
{
    ...
    Vertex *ver = graph.at(dest.get_name());
    path.push(dest);
    
    if (ver->predecessor == nullptr)
        return path;
    
    while (ver->predecessor != ver) {
        
        Artist pred = get_predecessor(ver->artist);
        path.push(pred);
        ver = graph.at(ver->predecessor->artist.get_name());
        
        if (ver == graph.at(source.get_name()))
            return path;
    }
